...
interface DetailDayData {
  memoLines: MemoLine[];
  todos: TodoItem[];
}

const EMPTY_LINES: MemoLine[] = Array.from({ length: 10 }, () => ({
  text: "",
  checked: false,
}));

function makeLines(lines: MemoLine[]): MemoLine[] {
...
const detailData: Record<number, DetailDayData> = {
  10: {
    memoLines: makeLines([
...
function renderInputList(day: number): void {
