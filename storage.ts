...
// 詳細画面の識別ID:T-020（ToDoリスト）に置く3種類のアイコン
// （🔲・🟦・☑️）のうち、その日に選択されているものを1件だけ
// 保存する窓口です。メモ本体（LinesStore）とは別の関心事のため、
// 独立したストレージキー（IconStore）で管理します。
// ※ 将来サーバーAPI化する場合も、この窓口だけを差し替えれば
//   済むようにしています。
// -----------------------------------------------------------

type IconStore = Record<string, string>;
...
function readIconStore(): IconStore {
  try {
    const raw = window.localStorage.getItem(ICON_STORAGE_KEY);
    return raw ? (JSON.parse(raw) as IconStore) : {};
  } catch {
    return {};
  }
}

function writeIconStore(store: IconStore): void {
  try {
...
const MemoStorage = {
  ...
  loadAllIcons(): Promise<Record<number, string>> {
    ...
    days.forEach((day) => {
      ...
      const lines = getLinesForDay(day);
      if (hasTodoItems(lines)) result[day] = "☑️";
    });
